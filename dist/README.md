...
import { WeissAccessibilityCenterComponent, WeissAccessibilityToggleDirective } from 'weiss-accessibility-center';
...
@Component({
  ...
  template: `
    <button class="usa-button" weissA11yToggle>
      Toggle Accessibility Center
    </button>
...
export class AppComponent {
  options = { displayType: 'panel' };
}
```

You can also add these imports at feature component level in a router-driven, `bootstrapApplication` setup.

### Legacy: NgModule Usage

### 1. Import the Module

Note: `WeissAccessibilityCenterModule` is provided as a legacy wrapper for apps still using NgModules. Prefer the standalone approach above for new integrations.

### Migration: NgModule → Standalone

Moving an existing app from the legacy NgModule wrapper to preferred standalone usage is straightforward.

1) Remove the module import and use standalone imports in your component.
...
@NgModule({
  ...
  imports: [BrowserModule, WeissAccessibilityCenterModule],
...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [WeissAccessibilityCenterComponent, WeissAccessibilityToggleDirective],
  template: `
    <button class="usa-button" weissA11yToggle>
      Toggle Accessibility Center
    </button>
...
If your app already uses `bootstrapApplication`:
...
Notes:
- Prefer standalone imports going forward; the NgModule wrapper remains only for legacy compatibility.
- Internal dependencies (`CommonModule`, `FormsModule`, `AsyncPipe`) are included by the library; you do not need to import them yourself when using standalone.
